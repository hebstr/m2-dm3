...
format:
  hebstr-template-html:
    code-tools:
      source: https://codeberg.org/hebstr/m2_dm3/src/branch/main/
...
add_script <- \(x, ...) add_code_file(here::here("scripts", x), ...)
```
...
`r add_code_file(".Rprofile", lang = "r")`
`r add_script("_setup.R")`

# Data management

```{r}
.labels_qmd <-
list(ptid = "identifiant patient",
...
opts$view$data |>
  filter(!is.na(n_miss),
         str_ends(variable, "weight|pam|fc|fr")) |>
  select(variable,
         label,
         n_ab = n_miss,
         p_ab = p_miss) |>
...
identical(sum(df_base$death == "Yes"),
...
df_base |>
  count(death_180 =
          case_when(as.numeric(dthdte - sadmdte) <= 180 ~ "Yes",
                    .default = "No"))
```
...
```{r}
.title_descr_base <-
map(c(demo = "variables sociodémographiques",
...
.title_uv_demo <-
str_fig(title = .title_descr$uv[1],
        acro = .acro,
        qmd = TRUE)
...
`r add_script("tbl_bv_ql.R", suffix = "(1)")`

```{r}
#| label: tbl-bv-ev-death-30-demo
#| tbl-cap: !expr ".title_descr$death[1]"

tbl_bv_ev_death_30_demo
```

```{r}
#| label: tbl-bv-ev-death-30-atcd
#| tbl-cap: !expr ".title_descr$death[2]"

tbl_bv_ev_death_30_atcd
```

```{r}
#| label: tbl-bv-ev-death-30-pv
#| tbl-cap: !expr ".title_descr$death[3]"

tbl_bv_ev_death_30_pv
```

```{r}
#| label: tbl-bv-ev-death-30-bio
#| tbl-cap: !expr ".title_descr$death[4]"

tbl_bv_ev_death_30_bio
```

`r add_script("tbl_bv_qt.R", suffix = "(1)")`

```{r}
#| label: tbl-bv-ev-death-30-bin
#| tbl-cap: !expr ".title_descr$death[5]"

tbl_bv_ev_death_30_bin
```

```{r}
#| label: tbl-bv-ev-death-30-qt
#| tbl-cap: !expr ".title_descr$death[6]"

tbl_bv_ev_death_30_qt
```

## Cathétérisme droit

`r add_script("tbl_bv_ql.R", suffix = "(2)")`

```{r}
#| label: tbl-bv-rhc-demo
...
```{r}
#| label: tbl-bv-rhc-bio
#| tbl-cap: !expr ".title_descr$rhc[4]"

tbl_bv_rhc_bio
```

`r add_script("tbl_bv_qt.R", suffix = "(2)")`

```{r}
#| label: tbl-bv-rhc-bin
...
.title_sp_dist <-
map(c(global = "chez",
      match = "après appariement entre"),
      ~ glue("Distribution du score de propension {.} les sujets
             {str_color('sans', .pal[1])} et {str_color('avec', .pal[2])}
             cathétérisme droit."))

.sp_dist_match <-
lst(cal = df_match_model$caliper,
    n_pair = sum(df_match$rhc == 0),
    total_pair = sum(df_setup$rhc == 1),
    p_match = percent(n_pair / total_pair, accuracy = 0.1))

.title_sp_dist_match <-
str_fig(title = .title_sp_dist$match,
        note =
          "Appariement 1:1 selon la méthode des plus proches voisins.
          Caliper fixé à {.sp_dist_match$cal} (brut). {.sp_dist_match$n_pair}
          appariements réalisés sur {.sp_dist_match$total_pair} possibles,
          soit avec une correspondance de {.sp_dist_match$p_match}.",
        qmd = TRUE)
```

La distribution du score de propension dans chaque groupe était la suivante

`r add_script("fig_dist.R", suffix = "(1)")`
...
include_graphics("output/fig_dist.global.png")
```
...
`r add_script("fig_dist.R", suffix = "(2)")`
...
include_graphics("output/fig_dist.match.png")
```
...
.title_smd <-
str_fig(title =
          "Différences moyennes standardisées (SMD) pour chaque variable
          constitutive du score de propension, {str_color('avant', .pal[1])}
          et {str_color('après', .pal[2])} appariement.",
        note =
          "{nrow(df_match) / 2} appariements sur le score propension
...
`r add_script("fig_surv.R")`

::: panel-tabset

```{r}
.title_surv <-
map(c(non_ajusté = "sans",
      ajusté = "après"),
    ~ str_fig(title =
                  glue("Comparaison du risque de mortalité à 30 jours
