...
.title_uv_demo <- str_fig(
  title = .title_descr$uv[1],
  acro = .acro,
  qmd = TRUE
)
...
df_sp["sp"] |>
  ...
  modify_header(stat_6 ~ str_glue("**{names(opts$qt_stat$mean)}**")) |>
  gt_qmd()
```
...
.title_surv <- map(
  ...
  ~ str_fig(
    title = str_glue(
      "Comparaison du risque de mortalité à 30 jours entre les sujets
      {str_color('sans', .pal[1])} et {str_color('avec', .pal[2])}
      cathétérisme droit, {.} ajustement."
    ),
