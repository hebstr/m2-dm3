fig_surv <- \(x, y) {
  ...
  lst(data =
      ...
      model =
        lst(fit =
              exprs(Surv(tt_death, ev_death_30) ~ rhc, data = data),
            tte =
              lst(obj = do.call("survfit2", fit),
                  tidy =
                    tidy_survfit(obj) |>
                      split(~ strata) |>
                      map(~ . |>
                            unite("n.event/censor", n.event:n.censor,
                                  sep = "/", remove = TRUE) |>
                            unite("cum.event/censor", cum.event:cum.censor,
                                  sep = "/", remove = TRUE) |>
                            merge_estim_ci(ci_data = opts$ci$data) |>
                            select(time, starts_with(c("n", "cum")), estimate_ci))),
            cox =
              lst(obj = do.call("coxph", fit),
                  tidy =
                    tidy(obj,
                         exponentiate = TRUE,
                         conf.int = TRUE) |>
                      merge_estim_ci(ci_data = opts$ci$data) |>
                      mutate(str = "Hazard ratio pour le décès",
  ...
  .surv$model$tte$obj |>
    ...
    geom_text(data = .surv$model$cox$tidy,
              mapping =
                aes(label =
                      glue("{str} {opts$ci$label}{opts$sep$int}{estimate_ci}
                           Log rank {p.value}"),
                    x = 30,
                    y = 1),
              size = 2.25,
    ...
    scale_ggsurvfit(x_scales =
                      list(name = "Durée jusqu'au décès (jours)",
                           breaks = seq(0, 30, by = 5),
                           expand = expansion(mult = 0.05)),
                    y_scales =
                      list(name = "Probabilité de survie (%)",
