...
df_base <- read_csv2("data/dm3.csv")[-1] |>
  set_names(tolower) |>
...
df_setup <- df_base |>
  ...
  easy_fct(sex, "Homme" = "Male", "Femme" = "Female") |>
  easy_fct(ethnic, "Blanche" = "White", "Noire" = "Black", "Autre" = "Other") |>
  easy_fct(
    cancer,
    "Non" = "No",
    "Localisé" = "Yes",
  ...
  easy_fct(age, 45, 75, .btw = 60, .name = "age_cat") |>
  easy_fct(educ, 9, 13) |>
  easy_fct(
    educ,
    "Élémentaire" = "<9",
...
use_vars(df_setup, .parametric = c("weight", "score_apa"))
...
.sp_recipe <- reformulate(sp_vars, "rhc") |>
  recipe(data = df_imp) |>
